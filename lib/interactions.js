    ...
    constructor(interactions) {
        ...
        this.interactions.client.on("raw", (event) => {
            ...
            if (cmdData) {
                ...
                const handler = (type) => {
                    console.warn(`User ${rawInteraction.member.user.username}#${rawInteraction.member.user.discriminator}(${rawInteraction.member.user.id}) attempted to use ${type} unregistered command '${rawInteraction.data ? rawInteraction.data.name : undefined}'`);
                ...
                this.interactions
                    ...
                    this.interactions
                        ...
                        .then(() => handler(`global`))
                        .catch((e) => {
                        console.warn(`Failed to remove unregistered command ${rawInteraction.data ? rawInteraction.data.name : undefined}, wasn't either a global or guild command`);
                    });
                });
            }
        });
